...
export class QueryBuilder {
  ...
  private buildSelect(): { query: string; params: any[] } {
    let query = `SELECT ${this.queryParts.columns?.join(", ")} FROM ${this.queryParts.table}`;
    if (this.queryParts.where) {
      query += " WHERE " + this.queryParts.where.map(cond => `${cond.column} ${cond.operator} ?`).join(" AND ");
  ...
  private buildInsert(): { query: string; params: any[] } {
    const columns = Object.keys(this.queryParts.set!);
    const placeholders = columns.map(() => "?").join(", ");
    const query = `INSERT INTO ${this.queryParts.table} (${columns.join(", ")}) VALUES (${placeholders})`;
    return { query, params: this.queryParts.values! };
  }

  private buildUpdate(): { query: string; params: any[] } {
    const setClause = Object.keys(this.queryParts.set!).map(column => `${column} = ?`).join(", ");
    let query = `UPDATE ${this.queryParts.table} SET ${setClause}`;
    if (this.queryParts.where) {
      query += " WHERE " + this.queryParts.where.map(cond => `${cond.column} ${cond.operator} ?`).join(" AND ");
      this.params.push(...this.queryParts.where.map(cond => cond.value));
    }
