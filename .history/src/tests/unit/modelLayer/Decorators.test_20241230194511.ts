import { assertEquals, assertThrows, ReflectMetadata } from "../../../deps.ts";
...
Deno.test("Decorators correctly register model and columns", () => {
	...
	assertEquals(idColumn?.type, "integer"); // Changed from "number" to "integer"
...
Deno.test("Column decorator sets correct metadata", () => {
  ModelRegistry.registerModel(TestUser);
  const metadata = ModelRegistry.getModelMetadata(TestUser);
  const idColumn = metadata.columns.find(c => c.propertyKey === "id") as ColumnMetadata;
