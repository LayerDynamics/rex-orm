...
Deno.test("QueryBuilder constructs and executes UPDATE queries", async () => {
  ...
  assertEquals(result.rowCount, 1);
  assertEquals(result.rows[0].query, "UPDATE users SET name = $1, email = $2 WHERE id = $3");
  assertEquals(result.rows[0].params, ["Jane Doe", "jane@example.com", 1]);
});
...
Deno.test("QueryBuilder constructs and executes DELETE queries", async () => {
  ...
  assertEquals(result.rowCount, 1);
  assertEquals(result.rows[0].query, "DELETE FROM users WHERE id = $1");
  assertEquals(result.rows[0].params, [1]);
});
