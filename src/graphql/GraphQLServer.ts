	...
	constructor(
		...
		this.schema=new graphql.GraphQLSchema({
			query: new graphql.GraphQLObjectType({
				name: 'Query',
				fields: () => ({
					...Object.entries(schemaConfig.queries).reduce((acc,[key,query]) => ({
						...acc,
						[key]: {
							type: query.type,
							args: query.args||{},
							resolve: query.resolve||(async () => null)
						}
					}),{})
				})
			}),
			mutation: schemaConfig.mutations&&Object.keys(schemaConfig.mutations).length>0
				? new graphql.GraphQLObjectType({
					name: 'Mutation',
					fields: () => ({
						...Object.entries(schemaConfig.mutations).reduce((acc,[key,mutation]) => ({
							...acc,
							[key]: {
								type: mutation.type,
								args: mutation.args||{},
								resolve: mutation.resolve||(async () => null)
							}
						}),{})
					})
				})
