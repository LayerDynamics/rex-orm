...
export class QueryBuilder {
  ...
  private buildUpdate(): { query: string; params: any[] } {
    ...
    if (this.queryParts.where && this.queryParts.where.length > 0) {
      query += " WHERE " + this.queryParts.where.map((cond, idx) => `${cond.column} ${cond.operator} $${Object.keys(this.queryParts.set!).length + idx + 1}`).join(" AND ");
      this.params.push(...this.queryParts.where.map(cond => cond.value));
    }
  ...
  async execute(adapter: DatabaseAdapter): Promise<QueryResult> {
    // Reset params array before building query
    this.params = [];
    ...
    // Reset query parts after execution
    this.queryParts = {};
